--[[ 
Removed The Thingy reason the script wont work but this is adjust one its not skidded leaked by some hacker credits to hacker 
--]]
...
local function MenuNameUpdate()
    while task.wait() do
        ...
        for i = 1, #Name do
            ...
            task.wait(0.6) -- fixed made it long bit
        end
    end
end
...
Menu.Slider('Main', 'Trigger Bot', 'Range', 1, 250, 2, '', 1, function(Value) -- 250 maxmium adjust
...
local TSSection = Menu.Container("Main", "Target Strafe", "Right") do
    ...
    Menu.Slider("Main", "Target Strafe", "Distance", 0, 999, 9, '', 1, function(a)
        Configurations.Misc.TargetStrafe.Distance = a
    end)
   -- these distance and other is adjusted 
    Menu.Slider("Main", "Target Strafe", "Speed", 0, 999, 1, '', 1, function(a)
        Configurations.Misc.TargetStrafe.Speed = a
    end)
    
    Menu.Slider("Main", "Target Strafe", "Height", 0, 999, 0, '', 1, function(a)
        Configurations.Misc.TargetStrafe.Height = a
    end)
    
    Menu.Slider("Main", "Target Strafe", "Randomization Value", 1, 999, 2, '', 1, function(Value) -- adjusted
...
Menu.Slider('Misc', 'CFrame Movement', 'Speed Amount', 1, 14, 1, '', 1, function(Value) -- adjusted kinda?
