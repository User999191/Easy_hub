...
getgenv().Settings = {
    ["FearLas"] = {
        ...
        ["Prediction"] = {
            ["Horizontal"] = 0.1985,
            ["Vertical"] = 0.4,
            ["Aimbot"] = 1.6, ----Set To 1 or this rigged
        },
        ["CamPrediction"] = {
            ["Prediction"] = {
                ["Horizontal"] = 0.185,
                ["Vertical"] = 0.1,
              ["Sticky_Aim"] = 2,
              ["Aimbot"] = 2, ----Set To 1 or this rigged
            },
        },
        ["NOTIF"] = true,
        ["AUTOPRED"] = false,
...
getgenv().ShakeY = 1
getgenv().ShakeZ = 0
getgenv().PREDICTION = 0.116869348534243
getgenv().JUMPPREDICTION = 2
...
function SendNotification(text)
    Notification:Notify(
        ...
        {OutlineColor = Color3.fromRGB(62.1041, 69.942, 13.2445),Time = 3, Type = "image"},
        {Image = "http://www.roblox.com/asset/?id=6023426923", ImageColor = Color3.fromRGB(62.1041, 69.942, 13.2445)}
    )
end
...
--// Change Prediction,  AutoPrediction Must Be Off
    ...
    function noob(player)
        ...
        local handler = makemarker(guimain, player.Character:WaitForChild(SelectedPart), Color3.fromRGB(107, 84, 255), 0.3, 3)
    ...
    spawn(function()
        ...
        placemarker.Transparency = 0.95
        if getgenv().Settings.FearLas.DOT then
        makemarker(placemarker, placemarker, Color3.fromRGB(15,100,255), 1, 0)
        end
    end)
            ...
            for _, Obj in next, Clone:GetDescendants() do
                ...
                elseif Obj:IsA("BasePart") or Obj:IsA("Meshpart") or Obj:IsA("Part") then
                    if Obj.Transparency == 1 then
                        ...
                        Obj.Size = Obj.Size + V3(0.15, 0.15, 0.15)
                    end
                end
...
UIGradient.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(115,0,0)), ColorSequenceKeypoint.new(1, Color3.fromRGB(155,40,0))}
UIGradient.Rotation = 90
UIGradient.Parent = Top

UIGradient_2.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(52, 52, 52)), ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 0, 0))}
...
UIGradient_3.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(58, 58, 58)), ColorSequenceKeypoint.new(1, Color3.fromRGB(30, 30, 30))}
...
UIGradient_4.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(184, 159, 227)), ColorSequenceKeypoint.new(1, Color3.fromRGB(102, 88, 156))}
...
local function GetStickyAimPrediction(Part)
    return Part.CFrame + Predict(Part.Velocity) * (getgenv().STICKYAIMPREDICTION)
end

local function GetAimbotPrediction(Part)
    return Part.CFrame + Predict(Part.Velocity) * (getgenv().AIMBOTPREDICTION)
end
...
LocalHL.OutlineColor = Color3.fromRGB(215,255,255)
...
do
	local Window = library:AddWindow("senselight.cc v1.5", {
		main_color = Color3.fromRGB(62.1041, 69.942, 13.2445),
...
Droeopdown:Add("Head")
	...
	local Lenabled = Tab:AddSwitch("Enabled", function(bool)
        ...
        local Lenabled = Tab:AddSwitch("Enable Sticky aim", function(bool)
		getgenv().Sticky_aim = bool
		end)
		Lenabled:Set(false)

        local Lenabled = Tab:AddSwitch("Enabled Aimbot", function(bool)
		getgenv().Aimbot = bool
		end)
		Lenabled:Set(false)
	...
	Tab:AddTextBox("Aimbot", function(text)
		getgenv().Aimbot = text
		end, 
        {
	["clear"] = false, -- Default: true (options are optional)
...
PredTypeThing:Add("Yun")
	...
	getgenv().VERTICALPREDICTION = text
		end, 
	{
	["clear"] = false, -- Default: true (options are optional)
	})
...
SoundsOptions:Add("mixkit-cartoon-dazzle-hit-and-birds-746")
...
local AntiFolder = Tib:AddFolder("More stuff")
		AntiFolder:AddLabel("Anti voids prevert falling")
               AntiFolder:AddButton("Anti void", function()
local part = Instance.new("Part")
part.Size = Vector3.new(50000, 4, 50000) -- Adjust size as needed
part.Position = Vector3.new(0, -50, 0) -- Adjust Y position to be below the void
part.Anchored = true
part.Color = Color3.new(100, 190, 70)

part.Parent = game.Workspace -- Change parent to appropriate location in your game hierarchy
end)

AntiFolder:AddLabel("makes hitbox size appear")
               AntiFolder:AddButton("Hitbox show", function()
...
TextButton.Name = "senselight v1.5"
TextButton.Parent = ScreenGui
TextButton.BackgroundColor3 = Color3.fromRGB(69,0,0)
TextButton.BackgroundTransparency = 0.5
TextButton.BorderSizePixel = 0
TextButton.Position = UDim2.new(1, -120, 0, 15) -- Adjusted the position to top right corner
TextButton.Size = UDim2.new(0, 110, 0, 17)
TextButton.Font = Enum.Font.SourceSans
TextButton.Text = "Rightclick"
TextButton.TextColor3 = Color3.fromRGB(25, 150, 255)
...
--// Quick Execute
	local Console = Tub:AddConsole({
		...
		HA:AddButton("Execute", function()
			loadstring(Console:Get())()		end)
		HA:AddButton("Clear", function()
			Console:Set("2255 set")
		end)
